...
class Runner:
    def __init__(self, config):
        ...
        self.capturer = TwoChannel(config.output_path, config.timestamp)
    ...
    def fetch_audio(self):
        def normalize_path(
            raw_path: str | Path, base_dir: str | Path | None = None
        ) -> Path:
...
class Config:
    def __init__(
        self, audio_file_path, skip_diarization, output_path, skip_hooks, hooks=[]
    ):
        ...
        self.output_path = (
            output_path if bool(output_path) else "~/tmp/transcriptions"
        )  # OUTPUT_DIR
        self.hooks = hooks
